...
//--------------Args--------------//
//按钮 
const int buttonPort = 10;//外接确认按钮端口 
//地灰 
...
const int mSpeed_3 = 50;
const int hSpeed_3 = 80;
const int tSpeed_3 = 60;
...
////=================SEQUENCE=================//
//子程序序列(0-49) 
int Sequence[50][100]= {{6,6,6,8,102},//单 航  
...
void drive(int lspd,int rspd){//设置速度 
	if(isMotoCL){
		SetMotoCL(lMotorPort,(invertL?-1:1)*lspd*lcoe);
		SetMotoCL(rMotorPort,(invertR?-1:1)*rspd*rcoe);
	}else{
		SetMoto(lMotorPort,(invertL?-1:1)*lspd*lcoe);
		SetMoto(rMotorPort,(invertR?-1:1)*rspd*rcoe);
	}
}
